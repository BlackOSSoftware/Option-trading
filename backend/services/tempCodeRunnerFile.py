...
def fetch_option_greek() -> None:
    ...
    manual_expiry = option.get("manualExpiry")
    if manual_expiry:
    ...
    # 🔹 DELTA SELECTION (AUTO / MANUAL) ← ADDED
    manual_delta = option.get("manualDelta")

    if manual_delta:
        target_delta = float(manual_delta)
        delta_source = "MANUAL"
    else:
        target_delta = float(option.get("targetDelta", 0.20))
        delta_source = "AUTO"
    ...
    body = {"name": option["name"], "expirydate": expiry}
    ...
    underlying, avg_iv, days_to_expiry = get_market_inputs(chain, expiry)
    ...
    nearest_ce, nearest_pe = find_nearest_delta(
        chain,
        spot=underlying,
        days=days_to_expiry,
    ...
    # GET TOKENS + LTP
    # ==============================
    call_token = find_token_from_candidates("NIFTY", expiry, ce_strike, "CE")
    put_token = find_token_from_candidates("NIFTY", expiry, pe_strike, "PE")
    ...
    # FIND HEDGE OPTIONS
    # ==============================
    hedge_ce_5rs = find_nearest_5rs_hedge_options(chain, ce_strike, "CE", user, expiry, underlying, days_to_expiry)
    hedge_pe_5rs = find_nearest_5rs_hedge_options(chain, pe_strike, "PE", user, expiry, underlying, days_to_expiry)

    result = {
    "targetDelta": target_delta,
    "deltaSource": delta_source,
    "expirySource": expiry_source,
